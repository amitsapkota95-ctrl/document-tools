...
const EDITOR_TEXTAREA_CLASS =
  "mt-0 block w-full min-h-[28rem] flex-1 resize-none border-0 bg-cream p-4 font-mono text-sm leading-relaxed text-forest placeholder:text-sand-light focus:outline-none focus:ring-2 focus:ring-inset focus:ring-sage/40";
...
function TextEditorPanel({
  ...
  return (
    <div className={`${CARD_CLASS} flex flex-col overflow-hidden border-t-2 shadow-eco ${accentClass}`}>
      <div className="flex items-center justify-between gap-3 border-b border-moss-dark/40 bg-moss-light/40 px-4 py-3">
        <div className="flex items-center gap-2">
          {icon}
          <span className="text-sm font-semibold text-forest">{title}</span>
        </div>
        <span className="shrink-0 text-xs text-sand-light">
...
function DiffPanel({ title, badge, accent, scrollRef, onScroll, children }: DiffPanelProps) {
  ...
  return (
    <div
      className={`${CARD_CLASS} flex min-h-0 flex-1 flex-col overflow-hidden border-t-2 shadow-eco ${accentClass}`}
    >
      <div className="flex shrink-0 items-center justify-between gap-3 border-b border-moss-dark/40 bg-moss-light/40 px-4 py-3">
        <div className="flex items-center gap-2">
          <span className="text-sm font-semibold text-forest">{title}</span>
          <span className="rounded-full border border-moss/60 bg-cream/80 px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wide text-sand">
            {badge}
          </span>
        </div>
      </div>
...
export default function CompareTextTool() {
  ...
  const textEditors = (
    <div className="grid w-full gap-4 lg:grid-cols-2">
      ...
      <TextEditorPanel
        ...
        icon={<FilePenLine className="h-4 w-4 text-sage" aria-hidden />}
      />
    </div>
  );
  ...
  const sidebarControls = (
    <div className="flex flex-col gap-5">
      ...
      {!showDiff ? (
        <>
          <p className="text-xs leading-relaxed text-sand-light">
          ...
          {diffStats && (
            <div className="space-y-2 text-sm">
              <div className="flex items-center justify-between">
                <span className="text-sand">Added</span>
                <span className="font-semibold text-emerald-800">{diffStats.added} words</span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-sand">Removed</span>
                <span className="font-semibold text-red-800">{diffStats.removed} words</span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-sand">Similarity</span>
                <span className="font-semibold text-forest">{diffStats.similarity}%</span>
              </div>
            </div>
          )}

          <div className="space-y-2.5 rounded-lg border border-moss/50 bg-cream/50 p-3">
            <p className="text-xs font-semibold uppercase tracking-wider text-sand">Legend</p>
            ...
            <div className="flex items-center gap-2 text-xs text-forest">
              <span className="h-2.5 w-2.5 shrink-0 rounded-sm bg-emerald-50 ring-1 ring-inset ring-sage/40" />
              Added text
            </div>
          </div>
    ...
    <ToolWorkflowLayout
      ...
      workspace={
        <div className="flex min-h-0 flex-1 flex-col gap-4 animate-fade-in">
          {diffStats && (
            <div className="flex shrink-0 flex-wrap items-center gap-2 rounded-xl border border-moss/50 bg-cream/60 px-4 py-3">
              ...
              <span className="rounded-full border border-moss/60 bg-moss-light/50 px-3 py-1 text-xs font-semibold text-forest">
