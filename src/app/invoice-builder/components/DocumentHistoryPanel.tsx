...
export function DocumentHistoryPanel({ onLoad }: DocumentHistoryPanelProps) {
  ...
  return (
    <div className="rounded-xl border border-cream-300/50 bg-cream-200/20">
      <button
        type="button"
        onClick={() => setOpen((v) => !v)}
        className="flex w-full items-center gap-2 px-4 py-3 text-left text-sm font-bold text-forest-700"
      >
        <History className="h-4 w-4" />
        Document history ({history.length})
      </button>
      {open ? (
        <div className="max-h-64 space-y-2 overflow-y-auto border-t border-cream-300 px-4 py-3">
          {history.length === 0 ? (
            <p className="text-xs text-ink/50">Exported documents appear here for quick reload.</p>
          ) : (
            history.map((doc) => {
              ...
              return (
                <div
                  key={doc.id}
                  className="flex items-center justify-between gap-2 rounded-lg border border-cream-300 bg-cream px-3 py-2"
                >
                  <button
                    type="button"
