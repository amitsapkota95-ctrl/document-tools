  ...
  return (
    <div className="space-y-4">
      <div className="text-center">
        <h2 className="font-serif text-xl font-semibold text-forest">Choose a QR code type</h2>
        ...
        {TYPE_ORDER.map((type) => {
          ...
          return (
            <button
              ...
              className={`${CARD_CLASS} group flex flex-col items-start gap-2 p-4 text-left transition hover:border-sage-dark hover:shadow-lg`}
            >
              <span className="flex h-10 w-10 items-center justify-center rounded-lg bg-moss-light text-forest group-hover:bg-sage/20">
                <Icon className="h-5 w-5" aria-hidden />
              </span>
              <span className="font-semibold text-forest">{QR_TYPE_LABELS[type]}</span>
              <span className="text-sm text-sand">{QR_TYPE_DESCRIPTIONS[type]}</span>
            </button>
          );
        })}
      </div>
          ...
          <button
            ...
            className={`flex flex-col items-center gap-1 rounded-lg border px-1.5 py-2 text-[10px] font-semibold leading-tight ${
              isActive
                ...
                : "border-moss-dark bg-moss-light/80 text-forest hover:border-sage-dark"
            }`}
          >
